...
interface GraphProps {
  ...
  nodeToolTipConfig?: any;
  edgeToolTipConfig?: any;
}
  ...
  const initGraph = (): Graph | undefined => {
    ...
    const defaultNodeTooltipConfig = {
      ...
      formatText: (model: any) => {
        ...
        const hasServiceOpened = model?.providerService?.some((y) =>
    ...
    const defaultEdgeTooltipConfig = {
      type: 'edge-tooltip',
      shouldBegin: (evt: any) => {
    ...
    defaultMode.push(props.nodeToolTipConfig || defaultNodeTooltipConfig);
    defaultMode.push(props.edgeToolTipConfig || defaultEdgeTooltipConfig);
    if (canDragNode) {
    ...
    if (graph) {
      data?.nodes?.forEach((x) => {
        ...
        if (nodeItem) {
          ...
          graph.updateItem(x.id, {
            ...x,
            expanderTypeState: nodeItem.getModel().expanderTypeState,
          });
        }
      });
