...
const NumberPicker = (props: IFieldMergeState) => {
  ...
  const { addonBefore, addonAfter, compact = true, disabled, ...rest } = componentProps;
  return (
    <Input.Group compact={compact} style={{ display: 'inline-flex', lineHeight: '32px' }}>
      {addonBefore}
      <InputNumber value={value} onChange={mutators.change} disabled={(!editable) || disabled} {...rest} />
      {addonAfter}
    </Input.Group>
  );
};
